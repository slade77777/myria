import React from 'react';

interface IProp {
  size?: number;
}

const CopyLinkIcon: React.FC<IProp> = ({ size }) => {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M16.4983 18.25H14.9883C14.5783 18.25 14.2383 17.91 14.2383 17.5C14.2383 17.09 14.5783 16.75 14.9883 16.75H16.4983C19.1183 16.75 21.2483 14.62 21.2483 12C21.2483 9.38 19.1183 7.25 16.4983 7.25H14.9983C14.5883 7.25 14.2483 6.91 14.2483 6.5C14.2483 6.09 14.5783 5.75 14.9983 5.75H16.4983C19.9483 5.75 22.7483 8.55 22.7483 12C22.7483 15.45 19.9483 18.25 16.4983 18.25Z"
        fill="#E7EBEE"
      />
      <path
        d="M9 18.25H7.5C4.05 18.25 1.25 15.45 1.25 12C1.25 8.55 4.05 5.75 7.5 5.75H9C9.41 5.75 9.75 6.09 9.75 6.5C9.75 6.91 9.41 7.25 9 7.25H7.5C4.88 7.25 2.75 9.38 2.75 12C2.75 14.62 4.88 16.75 7.5 16.75H9C9.41 16.75 9.75 17.09 9.75 17.5C9.75 17.91 9.41 18.25 9 18.25Z"
        fill="#E7EBEE"
      />
      <path
        d="M16 12.75H8C7.59 12.75 7.25 12.41 7.25 12C7.25 11.59 7.59 11.25 8 11.25H16C16.41 11.25 16.75 11.59 16.75 12C16.75 12.41 16.41 12.75 16 12.75Z"
        fill="#E7EBEE"
      />
    </svg>
  );
};

export default CopyLinkIcon;
