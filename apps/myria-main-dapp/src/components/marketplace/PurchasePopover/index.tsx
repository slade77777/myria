...
const PurchasePopover: FC<IProp> = ({ assetBuy, onCloseMessage, onConfirm }) => {
  ...
  const elementContent = useMemo(() => {
    ...
    if(statusPurchase === PurchaseStatus.SUCCESS){
      return {
        title: <span className='text-[24px]'><Trans>Purchase Successful!</Trans></span>,
        assetName: <span className='text-[14px]'>You have purchased <span className='text-base/10'>{assetBuy.name}</span></span>,
        ...
        history: <div className="flex justify-between mt-2">
        <span>
    ...
    if(statusPurchase === PurchaseStatus.FAIL){
      return {
        title: <span className='text-[24px]'><Trans>Purchase Failed!</Trans></span>,
        assetName: <span className='text-[14px]'>Your deposit could not be completed due to an error.</span>,
    ...
    // UI CHECKOUT
    return {
      title: <span className='text-[24px]'><Trans>Complete checkout</Trans></span>,
      assetName:<span className='text-[14px]'>Item {assetBuy.name}</span>,
      icon: <CartIcon color='#9AC9E3' size={64} />,
      action: <ActionButtonCheckout 
...
const ActionButtonCheckout: FC<ButtonProps> = ({onCloseMessage, isProgressPurchase, onConfirm}) => {
  return <>
